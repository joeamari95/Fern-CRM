...
const FIELDS: Field[] = [
  { name: "filingNumber", label: "Filing Number", kind: "text", placeholder: "e.g. NYSCEF Doc. No. 5" },
...
export default function CourtPage() {
  const { items, add, update, remove, ready } = useCollection<DocketEntry>("docket");
    ...
    <>
      ...
      >
        ...
        <span className="text-[12.5px] text-[var(--muted)]">
          Log NYSCEF / e-Courts docket entries as they post. Live auto-sync is a future addition.
        </span>
      </div>
      ...
      <Card>
        <SectionHeader
          ...
          right={<AddButton onClick={() => setEditing("new")} label="Add filing" />}
        />

        {!ready ? null : sorted.length === 0 ? (
          <EmptyState
            title="No docket entries yet"
            hint="Record filings from NYSCEF / e-Courts so the full docket lives in one place."
            onAdd={() => setEditing("new")}
            addLabel="Add your first filing"
          />
        ) : (
          <div className="flex flex-col">
            {sorted.map((d) => (
              <div
                key={d.id}
                className="flex items-start gap-4 py-3 border-b hairline last:border-0"
              >
                <div className="w-20 shrink-0">
                  {d.filingNumber ? (
                    <div className="tag">{d.filingNumber}</div>
                  ) : (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {d.notes && (
                    <p className="text-[12.5px] text-[var(--muted)] mt-1 leading-snug">{d.notes}</p>
                  )}
                </div>
        ...
        <Attachments section="court" />
      </Card>

      {editing && (
        <Modal
          title={editing === "new" ? "Add filing" : "Edit filing"}
