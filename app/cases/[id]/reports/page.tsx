...
const FIELDS: Field[] = [
  ...
  { name: "status", label: "Status", kind: "select", options: [{ value: "current", label: "Current" }, { value: "superseded", label: "Superseded" }] },
  { name: "body", label: "Report Body", kind: "textarea", placeholder: "Status, liability, damages, next steps…" },
];

export default function ReportsPage() {
  const { id } = useParams<{ id: string }>();
  const { items, add, update, remove, ready } = useCollection<ReportEntry>(caseKey(id, "reports"));
  ...
  return (
    <>
      <CaseHeader caseId={id} title="Reports" />
      <div className="flex items-center justify-between -mt-3 mb-5 gap-3">
        <p className="text-[13px] text-[var(--muted)]">Status reports to the client / carrier. Mark the latest “Current”.</p>
        <AddButton onClick={() => setEditing("new")} label="Add report" />
      </div>

      {!ready ? null : sorted.length === 0 ? (
        <EmptyState title="No reports yet" hint="Add an initial report, then supplemental reports as the matter develops." onAdd={() => setEditing("new")} addLabel="Add your first report" />
      ) : (
            ...
            <Card key={r.id}>
              <div className="flex items-start justify-between gap-3 mb-2">
                ...
                <div className="flex items-center gap-2">
                  <Pill accent={r.status === "current" ? "green" : "muted"}>{r.status === "current" ? "Current" : "Superseded"}</Pill>
                  <RowActions onEdit={() => setEditing(r)} onDelete={() => remove(r.id)} />
                </div>
              </div>
              {r.body && <p className="text-[13.5px] leading-relaxed text-[var(--muted)] whitespace-pre-wrap">{r.body}</p>}
            </Card>
          ))}
        </div>
      )}
      ...
      {editing && (
        <Modal title={editing === "new" ? "Add report" : "Edit report"} onClose={() => setEditing(null)}>
