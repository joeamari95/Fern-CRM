...
const FIELDS: Field[] = [
  ...
  },
  {
    ...
    kind: "select",
    options: [
      { value: "outgoing", label: "Outgoing (we serve)" },
      { value: "incoming", label: "Incoming (served on us)" },
    ],
  },
  { name: "dueDate", label: "Due Date", kind: "date" },
  {
    ...
    kind: "select",
    options: [
      { value: "to-draft", label: "To Draft" },
      { value: "to-serve", label: "To Serve" },
      { value: "served", label: "Served" },
      { value: "responses-due", label: "Responses Due" },
...
export default function DiscoveryPage() {
  const { items, add, update, remove, ready } = useCollection<DiscoveryItem>("discovery");
        ...
        {list.length === 0 ? (
          ...
          <div className="flex flex-col gap-2.5">
            {list.map((d) => (
              <div
                key={d.id}
                className={`card-2 p-3.5 accent-bar bar-${statusAccent[d.status] === "green" ? "teal" : statusAccent[d.status]} pl-4`}
              >
                ...
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {d.dueDate && (
                    <span className="text-[11px] text-[var(--faint)]">
                      {fmtDate(d.dueDate)}
                      {relativeDue(d.dueDate) && ` · ${relativeDue(d.dueDate)}`}
                    </span>
                  )}
                </div>
              </div>
    ...
    <>
      ...
      {!ready ? null : items.length === 0 ? (
        <EmptyState
          ...
          hint="Track outgoing demands you serve and incoming demands you must answer."
          onAdd={() => setEditing("new")}
          addLabel="Add your first discovery item"
        />
      ) : (
      ...
      <div className="mt-5">
        <Card>
          ...
          <Attachments section="discovery" />
        </Card>
      </div>

      {editing && (
        <Modal
          title={editing === "new" ? "Add discovery item" : "Edit discovery item"}
